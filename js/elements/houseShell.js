...
import { matSiding, matTrim, matGround, matFloor, matGlass, matFrame, matWood, matMetal, matRoof } from '../materials.js';
...
export function createHouseShell() {
    ...
    grass.position.set(GRASS_CENTER_X, -0.02, GRASS_CENTER_Z);
    grass.receiveShadow = true;
    house.add(grass);

    // 建筑底部（地板色，覆盖草地中间）
    const bldgW = EAST_X - WEST_X;
    const bldgD = NORTH_Z - SOUTH_Z;
    const ground = new THREE.Mesh(
        new THREE.PlaneGeometry(bldgW, bldgD),
        matFloor,
    );
