...
3. 楼梯（原模型自带白色踏步，北墙 y≈4.0 东西向直跑）：
   ...
   - 第二跑 2F(x≈-5.4,z=3.19) -> 阁楼(z=6.17)，原模型完整
     （15 级踏步 + 顶部平台 x -0.81..-0.53，平台东侧即阁楼楼板
     x -0.51..3.66；探针实测整段净空 ≥1.6m，只缺 WALK 面）
   路线：门廊 -> 翼门 -> 翼 1F -> 后厅 -> 第一跑 -> 2F 楼板
   -> 中厅 2F -> 东翼门洞(x≈3.57,y≈-0.6)；
   2F 西端 -> 第二跑 -> 顶部平台 -> 阁楼。
   新增踏步为可见几何（MAT_wall 白色，与原踏步一致），两跑各配
...
# ── 第二跑 2F -> 阁楼：原模型踏步完整（15 级 + 顶部平台），只配 WALK 面 ──
# 踏面线实测：首踏中心(x=-5.21,z=3.26) -> 末踏中心(x=-0.97,z=5.99)，
# 平台板(x -0.81..-0.53, z=6.19)，东侧紧接阁楼楼板(x -0.51.., z=6.17)
A2_X0, A2_Z0 = -5.51, 3.07   # 坡面西端（踏面线延长到 2F 楼板上方）
A2_X1, A2_Z1 = -0.83, 6.08   # 坡面东端（平台西缘，+0.08 抬升后与平台齐平）
...
def main():
    ...
    # 注意 y 上限 4.9：北墙(y≈4.96)中厅左窗(x≈-1.1)的拱顶格栅面心
    # 在 y 4.94..4.97，y_max=5.05 时会被误删（窗框缺一块就是这来的）；
    # 井道内部构件都在 y<4.9，墙表面层不应进 VOID。
    VOIDS = [
        (-2.4, -0.4, 3.0, 4.9, 2.3, 3.2),
    ...
    # ── 2.5 第二跑 2F -> 阁楼：原踏步完整，配 WALK 坡面 + 平台过渡面 ──
    # 踏步/侧板均为 WALL_01 障碍：踏面顶在坡面线下方 ~0.08，不触发净空
    # 剔除（与第一跑同理）；坡面 y 范围避开两侧斜侧板(y 3.13..3.21 /
    # 4.80..4.88)，北檐柱(ROOF_01, y>=4.90) 膨胀会削掉坡面北缘几格，
    # 南墙(y≈3.19) 膨胀削掉南缘，剩余有效宽度仍 >0.9m。
    make_walk_slope_x('WALK_stairs_2f_attic', WS_Y[0], WS_Y[1],
                      A2_X0, A2_Z0, A2_X1, A2_Z1, floor_mat)
    # 平台 -> 阁楼楼板过渡面：覆盖平台板(z=6.19)、托梁缝(x -0.53..-0.51)
    # 与阁楼楼板(z=6.17) 的高差缝隙，保证坡面 -> 阁楼连通。
    make_walk_slope_x('WALK_stairs_atticout', 3.30, 4.70,
                      -1.05, 6.11, -0.30, 6.11, floor_mat)
    print('第二跑: WALK 坡面(2F->阁楼平台) + 平台过渡面')
