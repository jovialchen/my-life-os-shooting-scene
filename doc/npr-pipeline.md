...
- 厨卫硬面（FIXTURE_GLSL）：`MAT_cabinet`/`MAT_ceramic`/`MAT_counter`/`MAT_metal`/
  `MAT_bluegray`/`MAT_dark`/`MAT_enamel`/`MAT_stool_*`（changjing 厨房/卫浴家具）
  3 阶假光影（明暗差比布艺强一档、亮阶更窄 = 硬高光感）+ 釉面细颗粒
  （uFixShade/uFixGrain）；`MAT_trim` 并入 wood 变体
- 以上两组调参 `setInkInterior({woodShade, woodFine, fabShade, fabGrain, fixShade, fixGrain})`
